...
NSArray *getInterElementSpaces()
{
    ...
    if (!interElementSpaceArray) {
        interElementSpaceArray = [[NSArray alloc] initWithArray:@[
                                   // @[ordinary, operator, binary, relation, open, close, punct, fraction].
                                   // ordinary
                                   @[@(kMTSpaceNone), @(kMTSpaceThin), @(kMTSpaceNSMedium), @(kMTSpaceNSThick),
                                     @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNSThin)],
                                   // operator
                                   @[@(kMTSpaceThin), @(kMTSpaceThin), @(kMTSpaceInvalid), @(kMTSpaceNSThick),
                                     @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNSThin)],
                                   // binary
                                   @[@(kMTSpaceNSMedium), @(kMTSpaceNSMedium), @(kMTSpaceInvalid), @(kMTSpaceInvalid),
                                     @(kMTSpaceNSMedium), @(kMTSpaceInvalid), @(kMTSpaceInvalid), @(kMTSpaceNSMedium)],
                                   // relation
                                   @[@(kMTSpaceNSThick), @(kMTSpaceNSThick), @(kMTSpaceInvalid), @(kMTSpaceNone),
                                     @(kMTSpaceNSThick), @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNSThick)],
                                   // open
                                   @[@(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceInvalid), @(kMTSpaceNone),
                                     @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNone)],
                                   // close
                                   @[@(kMTSpaceNone), @(kMTSpaceThin), @(kMTSpaceNSMedium), @(kMTSpaceNSThick),
                                     @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNSThin)],
                                   // punct
                                   @[@(kMTSpaceNSThin), @(kMTSpaceNSThin), @(kMTSpaceInvalid), @(kMTSpaceNSThin),
                                     @(kMTSpaceNSThin), @(kMTSpaceNSThin), @(kMTSpaceNSThin), @(kMTSpaceNSThin)],
                                   // fraction
                                   @[@(kMTSpaceNSThin), @(kMTSpaceThin), @(kMTSpaceNSMedium), @(kMTSpaceNSThick),
                                     @(kMTSpaceNSThin), @(kMTSpaceNone), @(kMTSpaceNSThin), @(kMTSpaceNSThin)],
                                   // radical
                                   @[@(kMTSpaceNSMedium), @(kMTSpaceNSThin), @(kMTSpaceNSMedium), @(kMTSpaceNSThick),
                                     @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNone), @(kMTSpaceNSThin)]
                                   ]];
    }
...
UTF32Char getItalicized(unichar ch)
{
    ...
    if (ch == 'h') {
        ...
        @throw [NSException exceptionWithName:@"IllegalCharacter" reason:[NSString stringWithFormat:@"Unknown character %d used as variable.", ch] userInfo:nil];
    }
    return unicode;
}
...
{
    MTFont *_font;
    __strong NSMutableArray *_displayAtoms;
    CGPoint _currentPosition;
    __strong NSMutableAttributedString *_currentLine;
...
- (instancetype)initWithFont:(MTFont *)font style:(MTLineStyle)style cramped:(BOOL)cramped spaced:(BOOL)spaced
{
    ...
    if (self) {
        _font = [font retain];
        _displayAtoms = [[NSMutableArray alloc] init];
        _currentPosition = CGPointZero;
        _cramped = cramped;
        _spaced = spaced;
        _currentLine = [[NSMutableAttributedString alloc] initWithString:@""];
        _currentAtoms = [NSMutableArray new];
        self.style = style;
...
{
    [_font release];
    [_displayAtoms release];
    [_currentLine release];
    [_currentAtoms release];
    [_styleFont release];
    [super dealloc];
}
        ...
        switch (atom.type) {
            ...
            case kMTMathAtomPunctuation: {
                ...
                if (prevNode) {
                    ...
                    if (_currentLine.length > 0) {
                        if (interElementSpace > 0) {
                            ...
                            [_currentLine addAttribute:(NSString *)kCTKernAttributeName value:[NSNumber numberWithFloat:interElementSpace] range:[_currentLine.string rangeOfComposedCharacterSequenceAtIndex:_currentLine.length - 1]];
                        }
                    } else {
                        // increase the space
                        _currentPosition.x += interElementSpace;
                ...
                if (atom.type == kMTMathAtomPlaceholder) {
                    ...
                    current = [[NSAttributedString alloc] initWithString:atom.nucleus attributes:@{(NSString*) kCTForegroundColorAttributeName: (id)color.CGColor}];
                } else {
...
{
    // add the font
    [_currentLine addAttribute:(NSString *)kCTFontAttributeName value:(id)_styleFont.ctFont range:NSMakeRange(0, _currentLine.length)];
    ...
    _currentAtoms = [NSMutableArray new];
    _currentLineIndexRange = NSMakeRange(NSNotFound, NSNotFound);
    return [displayAtom autorelease];
}
...
- (MTDisplay *)makeFraction:(MTFraction *)frac
{
    ...
    MTFractionDisplay *display = [[MTFractionDisplay alloc] initWithNumerator:numeratorDisplay denominator:denominatorDisplay position:_currentPosition range:frac.indexRange];
    ...
    if (!frac.leftDelimiter && !frac.rightDelimiter) {
        return [display autorelease];
    } else {
        MTDisplay *retVal = [self addDelimitersToFractionDisplay:display forFraction:frac];
        [display release];
        return retVal;
    }
}
...
- (MTDisplay *)addDelimitersToFractionDisplay:(MTFractionDisplay *)display forFraction:(MTFraction *)frac
{
    ...
    [innerElements release];
    innerDisplay.position = _currentPosition;
    return [innerDisplay autorelease];
}
...
- (MTDisplay <DownShift> *)getRadicalGlyphWithHeight:(CGFloat)radicalHeight
{
    ...
    MTDisplay <DownShift> *glyphDisplay = nil;
...
{
    NSArray *variants = [_styleFont.mathTable getVerticalVariantsForGlyph:glyph];
    CFIndex numVariants = variants.count;
    CGGlyph glyphs[numVariants];
    for (CFIndex i = 0; i < numVariants; i++) {
        CGGlyph glyph = [(NSNumber *)variants[i] shortValue];
        glyphs[i] = glyph;
    }
...
{
    NSArray *parts = [_styleFont.mathTable getVerticalGlyphAssemblyForGlyph:glyph];
    if (parts.count == 0) {
        return nil;
    }
    NSArray *glyphs, *offsets;
    CGFloat height;
    [self constructGlyphWithParts:parts height:glyphHeight glyphs:&glyphs offsets:&offsets height:&height];
    CGGlyph first = ((NSNumber *)glyphs[0]).shortValue;
...
- (void)constructGlyphWithParts:(NSArray *)parts height:(CGFloat)glyphHeight glyphs:(NSArray **)glyphs offsets:(NSArray **)offsets height:(CGFloat *)height
{
    NSParameterAssert(glyphs);
    NSParameterAssert(offsets);
    
    for (int numExtenders = 0; true; numExtenders++) {
        NSMutableArray *glyphsRv = [NSMutableArray array];
        NSMutableArray *offsetsRv = [NSMutableArray array];
        ...
        if (minHeight >= glyphHeight) {
            ...
            *height = minHeight;
            ...
            for (int i = 0; i < offsetsRv.count; i++) {
                CGFloat offset = ((NSNumber *)offsetsRv[i]).floatValue + i*deltaIncrease;
...
- (MTDisplay *)makeLargeOp:(MTLargeOperator *)op
{
    ...
    if (op.nucleus.length == 1) {
        ...
        return [self addLimitsToDisplay:[glyphDisplay autorelease] forOperator:op delta:delta];
    } else {
        // Create a regular node
        NSMutableAttributedString *line = [[NSMutableAttributedString alloc] initWithString:op.nucleus];
        // add the font
        [line addAttribute:(NSString *)kCTFontAttributeName value:(id)_styleFont.ctFont range:NSMakeRange(0, line.length)];
        ...
        [line release];
        return [self addLimitsToDisplay:[displayAtom autorelease] forOperator:op delta:0];
    }
}
...
- (MTDisplay *)makeUnderline:(MTUnderLine *)under
{
    ...
    return [underDisplay autorelease];
}
...
- (MTDisplay *)makeOverline:(MTOverLine *)over
{
    ...
    return [overDisplay autorelease];
}
...
{
    NSArray *variants = [_styleFont.mathTable getHorizontalVariantsForGlyph:glyph];
    CFIndex numVariants = variants.count;
...
- (MTDisplay *)makeTable:(MTMathTable *)table
{
    ...
    if (numColumns == 0 || table.numRows == 0) {
        // Empty table
        return [[[MTMathListDisplay alloc] initWithDisplays:[NSArray array] range:table.indexRange] autorelease];
    }
    ...
    NSArray *displays = [self typesetCells:table columnWidths:columnWidths];
    
    // Position all the columns in each row
    NSMutableArray *rowDisplays = [NSMutableArray arrayWithCapacity:table.cells.count];
    for (NSArray *row in displays) {
    ...
    return [tableDisplay autorelease];
}

// Typeset every cell in the table. As a side-effect calculate the max column width of each column.
- (NSArray *)typesetCells:(MTMathTable *)table columnWidths:(CGFloat[])columnWidths
{
    NSMutableArray *displays = [NSMutableArray arrayWithCapacity:table.numRows];
    
    for(NSArray *row in table.cells) {
        NSMutableArray *colDisplays = [NSMutableArray arrayWithCapacity:row.count];
        ...
        for (int i = 0; i < row.count; i++) {
            ...
            columnWidths[i] = MAX(disp.width, 0.0f);
...
- (MTMathListDisplay *)makeRowWithColumns:(NSArray *)cols forTable:(MTMathTable *)table columnWidths:(CGFloat[])columnWidths
{
    ...
    return [rowDisplay autorelease];
}

- (void)positionRows:(NSArray *)rows forTable:(MTMathTable *)table
{
    // Position the rows
