...
//    NSImage *img = UIGraphicsGetImageFromCurrentImageContext();
//    return img;
//}
...
{
    NSArray *runs = (NSArray *)(CTLineGetGlyphRuns(_line));
    for (id obj in runs) {
...
- (void)setTextColor:(NSColor *)textColor
{
    ...
    [super setTextColor:textColor];
    for (MTDisplay *displayAtom in _subDisplays) {
...
- (instancetype)initWitRadicand:(MTMathListDisplay *)radicand glpyh:(MTDisplay *)glyph position:(CGPoint)position range:(NSRange)range
{
    self = [super init];
    if (self) {
        _radicand = [radicand retain];
        _radicalGlyph = [glyph retain];
        _radicalShift = 0;
...
- (instancetype)initWithGlyphs:(NSArray *)glyphs offsets:(NSArray *)offsets font:(MTFont *)font
{
    ...
    if (self != nil) {
        ...
        for (int i = 0; i < _numGlyphs; i++) {
            _glyphs[i] = ((NSNumber *)glyphs[i]).shortValue;
            _positions[i] = CGPointMake(0, ((NSNumber *)offsets[i]).floatValue);
        }
        _font = [font retain];
        self.position = CGPointZero;
    }
...
- (instancetype)initWithNucleus:(MTDisplay *)nucleus upperLimit:(MTMathListDisplay *)upperLimit lowerLimit:(MTMathListDisplay *)lowerLimit limitShift:(CGFloat)limitShift extraPadding:(CGFloat)extraPadding
{
    self = [super init];
    if (self != nil) {
        _upperLimit = [upperLimit retain];
        _lowerLimit = [lowerLimit retain];
        _nucleus = [nucleus retain];
...
- (void)updateLowerLimitPosition
{
    if (self.lowerLimit) {
        ...
        self.lowerLimit.position = CGPointMake(self.position.x - _limitShift + (self.width - _lowerLimit.width) / 2, self.position.y - _nucleus.descent - _lowerLimitGap - self.lowerLimit.ascent);
    }
}
...
- (instancetype)initWithAccent:(MTGlyphDisplay *)glyph accentee:(MTMathListDisplay *)accentee range:(NSRange)range
{
    self = [super init];
    if (self) {
        _accent = [glyph retain];
        _accentee = [accentee retain];
