...
{
    return [[[MTLargeOperator alloc] initWithValue:name limits:limits] autorelease];
}
...
+ (void)addLatexSymbol:(NSString *)name value:(MTMathAtom *)atom
{
    ...
    commands[name] = atom;
    if (atom.nucleus.length != 0) {
        NSMutableDictionary *dict = [self textToLatexSymbolNames];
        dict[atom.nucleus] = name;
    }
}

+ (NSArray *)supportedLatexSymbolNames
{
...
{
    NSDictionary *accents = [MTMathAtomFactory accents];
    NSString *accentValue = accents[accentName];
    if (accentValue) {
        return [[[MTAccent alloc] initWithValue:accentValue] autorelease];
    } else {
        return nil;
    }
}
...
{
    NSDictionary *delims = [MTMathAtomFactory delimiters];
...
+ (MTMathAtom *)tableWithEnvironment:(NSString *)env rows:(NSArray *)rows error:(NSError **)error
{
    ...
    static NSDictionary *matrixEnvs = nil;
    if (!matrixEnvs) {
        matrixEnvs = [[NSDictionary alloc] initWithDictionary:@{@"matrix": @[],
                       @"pmatrix": @[ @"(", @")"],
                       @"bmatrix": @[ @"[", @"]"],
                       @"Bmatrix": @[ @"{", @"}"],
                       @"vmatrix": @[ @"vert", @"vert"],
...
+ (NSMutableDictionary *)supportedLatexSymbols
{
    static NSMutableDictionary *commands = nil;
    if (!commands) {
        commands = [[NSMutableDictionary alloc] initWithDictionary:@{
                    ...
                    // Spacing
                    @",": [MTMathSpace spaceWithSizeInMu:3],
                    @">": [MTMathSpace spaceWithSizeInMu:4],
                    @";": [MTMathSpace spaceWithSizeInMu:5],
                    @"!": [MTMathSpace spaceWithSizeInMu:-3],
                    @"quad": [MTMathSpace spaceWithSizeInMu:18],  // quad = 1em = 18mu
                    @"qquad": [MTMathSpace spaceWithSizeInMu:36], // qquad = 2em
                    
                    // Style
                    @"displaystyle": [MTMathStyle styleWithStyleConstant:kMTLineStyleDisplay],
                    @"textstyle": [MTMathStyle styleWithStyleConstant:kMTLineStyleText],
                    @"scriptstyle": [MTMathStyle styleWithStyleConstant:kMTLineStyleScript],
                    @"scriptscriptstyle": [MTMathStyle styleWithStyleConstant:kMTLineStyleScriptScript]
                    }];
...
+ (NSDictionary *)aliases
{
    ...
    if (!aliases) {
        aliases = [[NSDictionary alloc] initWithDictionary:@{
                    @"lnot": @"neg",
                    @"land": @"wedge",
                    @"lor": @"vee",
                    @"ne": @"neq",
...
+ (NSMutableDictionary *)textToLatexSymbolNames
{
    static NSMutableDictionary *textToCommands = nil;
    if (!textToCommands) {
        NSDictionary *commands = [self supportedLatexSymbols];
        textToCommands = [[NSMutableDictionary alloc] initWithCapacity:commands.count];
...
+ (NSDictionary *)accents
{
    ...
    if (!accents) {
        accents = [[NSDictionary alloc] initWithDictionary:@{
                    @"grave": @"\u0300",
                    @"acute": @"\u0301",
...
+(NSDictionary  *)delimiters
{
    static NSDictionary *delims = nil;
    if (!delims) {
        delims = [[NSDictionary alloc] initWithDictionary:@{
                   @".": @"", // . means no delimiter
                   @"(": @"(",
                   @")": @")",
                   @"[": @"[",
