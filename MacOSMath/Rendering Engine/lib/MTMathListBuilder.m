...
{
    MTMathList *list = [[[MTMathList alloc] init] autorelease];
...
{
    static NSSet *singleCharCommands = nil;
...
- (MTMathAtom *)atomForCommand:(NSString *)command
{
    ...
    if (accent) {
        ...
        _currentInnerAtom = [[MTInner alloc] init];
...
{
    static NSDictionary *fractionCommands = nil;
    if (!fractionCommands) {
        fractionCommands = [[NSDictionary alloc] initWithDictionary:@{
                            @"over": @[], @"atop": @[], @"choose": @[@"(", @")"],
                            @"brack": @[@"[", @"]"], @"brace": @[@"{", @"}"]}];
    }
...
+ (NSDictionary *)spaceToCommands
{
    ...
    if (!spaceToCommands) {
        spaceToCommands = [[NSDictionary alloc] initWithDictionary:@{@3: @",", @4: @">", @5: @";", @(-3): @"!", @18: @"quad", @36: @"qquad"}];
    }
...
+ (NSDictionary *)styleToCommands
{
    ...
    if (!styleToCommands) {
        styleToCommands = [[NSDictionary alloc] initWithDictionary:@{@(kMTLineStyleDisplay): @"displaystyle", @(kMTLineStyleText): @"textstyle", @(kMTLineStyleScript): @"scriptstyle", @(kMTLineStyleScriptScript): @"scriptscriptstyle"}];
    }
...
{
    MTMathListBuilder *builder = [[[MTMathListBuilder alloc] initWithString:str] autorelease];
            ...
            for (int i = 0; i < table.numRows; i++) {
                ...
                for (int j = 0; j < row.count; j++) {
                    ...
                    if ([table.environment isEqualToString:@"matrix"]) {
                        if (cell.atoms.count >= 1 && ((MTMathAtom *)cell.atoms[0]).type == kMTMathAtomStyle) {
                    ...
                    if ([table.environment isEqualToString:@"eqalign"] || [table.environment isEqualToString:@"aligned"] || [table.environment isEqualToString:@"split"]) {
                        if (j == 1 && cell.atoms.count >= 1 && ((MTMathAtom *)cell.atoms[0]).type == kMTMathAtomOrdinary && ((MTMathAtom *)cell.atoms[0]).nucleus.length == 0) {
