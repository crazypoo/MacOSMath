...
+ (instancetype)atomWithType:(MTMathAtomType)type value:(NSString *)value
{
    switch (type) {
        case kMTMathAtomFraction:
            return [[[MTFraction alloc] init] autorelease];
            
        case kMTMathAtomPlaceholder:
            // A placeholder is created with a white square.
            return [[[[self class] alloc] initWithType:kMTMathAtomPlaceholder value:@"\u25A1"] autorelease];
            
        case kMTMathAtomRadical:
            return [[[MTRadical alloc] init] autorelease];
            
        case kMTMathAtomLargeOperator:
            // Default setting of limits is true
            return [[[MTLargeOperator alloc] initWithValue:value limits:YES] autorelease];
            
        case kMTMathAtomInner:
            return [[[MTInner alloc] init] autorelease];
            
        case kMTMathAtomOverline:
            return [[[MTOverLine alloc] init] autorelease];
            
        case kMTMathAtomUnderline:
            return [[[MTUnderLine alloc] init] autorelease];
            
        case kMTMathAtomAccent:
            return [[[MTAccent alloc] initWithValue:value] autorelease];
            
        case kMTMathAtomSpace:
            return [[[MTMathSpace alloc] initWithSpace:0] autorelease];
            
        default:
            return [[[MTMathAtom alloc] initWithType:type value:value] autorelease];
    }
}

- (instancetype)init
{
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTMathAtom init] cannot be called. Use [MTMathAtom initWithType:value:] instead." userInfo:nil];
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    atom.subScript = [[self.subScript copyWithZone:zone] autorelease];
    atom.superScript = [[self.superScript copyWithZone:zone] autorelease];
...
- (void)setSubScript:(MTMathList *)subScript
{
    if (subScript && !self.scriptsAllowed) {
        @throw [NSException exceptionWithName:@"Error" reason:[NSString stringWithFormat:@"Subscripts not allowed for atom of type %@", typeToText(self.type)] userInfo:nil];
    }
    [_subScript release];
    _subScript = [subScript retain];
}

- (void)setSuperScript:(MTMathList *)superScript
{
    if (superScript && !self.scriptsAllowed) {
        @throw [NSException exceptionWithName:@"Error" reason:[NSString stringWithFormat:@"Superscripts not allowed for atom of type %@", typeToText(self.type)] userInfo:nil];
    }
    [_superScript release];
    _superScript = [superScript retain];
}
...
- (void)fuse:(MTMathAtom *)atom
{
    ...
    if (!_fusedAtoms) {
        _fusedAtoms = [[NSMutableArray alloc] initWithObjects:[[self copy] autorelease], nil];
    }
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTFraction initWithType:value:] cannot be called. Use [MTFraction init] instead." userInfo:nil];
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    frac.numerator = [[self.numerator copyWithZone:zone] autorelease];
    frac.denominator = [[self.denominator copyWithZone:zone] autorelease];
    frac->_hasRule = self.hasRule;
    frac.leftDelimiter = [[self.leftDelimiter copyWithZone:zone] autorelease];
    frac.rightDelimiter = [[self.rightDelimiter copyWithZone:zone] autorelease];
    return frac;
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTRadical initWithType:value:] cannot be called. Use [MTRadical init] instead." userInfo:nil];
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    rad.radicand = [[self.radicand copyWithZone:zone] autorelease];
    rad.degree = [[self.degree copyWithZone:zone] autorelease];
    return rad;
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTLargeOperator initWithType:value:] cannot be called. Use [MTLargeOperator initWithValue:limits:] instead." userInfo:nil];
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTInner initWithType:value:] cannot be called. Use [MTInner init] instead." userInfo:nil];
}

- (void)dealloc
{
    [_leftBoundary release];
    [_rightBoundary release];
    [super dealloc];
}
...
- (void)setLeftBoundary:(MTMathAtom *)leftBoundary
{
    if (leftBoundary && leftBoundary.type != kMTMathAtomBoundary) {
        @throw [NSException exceptionWithName:@"Error" reason:[NSString stringWithFormat:@"Left boundary must be of type kMTMathAtomBoundary"] userInfo:nil];
    }
    [_leftBoundary release];
    _leftBoundary = [leftBoundary retain];
}
...
- (void)setRightBoundary:(MTMathAtom *)rightBoundary
{
    ...
    [_rightBoundary release];
    _rightBoundary = [rightBoundary retain];
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    inner.innerList = [[self.innerList copyWithZone:zone] autorelease];
    inner.leftBoundary = [[self.leftBoundary copyWithZone:zone] autorelease];
    inner.rightBoundary = [[self.rightBoundary copyWithZone:zone] autorelease];
    return inner;
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTOverline initWithType:value:] cannot be called. Use [MTOverline init] instead." userInfo:nil];
}

- (id)copyWithZone:(NSZone *)zone
{
    MTOverLine* op = [super copyWithZone:zone];
    op.innerList = [[self.innerList copyWithZone:zone] autorelease];
    return op;
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTUnderline initWithType:value:] cannot be called. Use [MTUnderline init] instead." userInfo:nil];
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTAccent initWithType:value:] cannot be called. Use [MTAccent initWithValue:] instead." userInfo:nil];
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTMathSpace initWithType:value:] cannot be called. Use [MTMathSpace initWithSpace:] instead." userInfo:nil];
}

+ (instancetype)spaceWithSizeInMu:(CGFloat)mu
{
    return [[[MTMathSpace alloc] initWithSpace:mu] autorelease];
}
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTMathStyle initWithType:value:] cannot be called. Use [MTMathStyle initWithStyle:] instead." userInfo:nil];
}

+ (instancetype)styleWithStyleConstant:(MTLineStyle)style
{
    return [[[MTMathStyle alloc] initWithStyle:style] autorelease];
}
...
@property (nonatomic) NSMutableArray *alignments;//<NSNumber*>
@property (nonatomic) NSMutableArray *cells;//<NSMutableArray<MTMathList*>*>
...
- (instancetype)initWithEnvironment:(NSString *)env
{
    ...
    if (self) {
        _alignments = [[NSMutableArray alloc] init];
        _cells = [[NSMutableArray alloc] init];
        _interRowAdditionalSpacing = 0;
        _interColumnSpacing = 0;
...
- (instancetype)initWithType:(MTMathAtomType)type value:(NSString *)value
{
    ...
    @throw [NSException exceptionWithName:@"InvalidMethod" reason:@"[MTMathTable initWithType:value:] cannot be called. Use [MTMathTable init] instead." userInfo:nil];
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    for (NSMutableArray *row in self.cells) {
        [cellCopy addObject:[[[NSMutableArray alloc] initWithArray:row copyItems:YES] autorelease]];
    }
    op.cells = cellCopy;
    return op;
}
...
- (instancetype)finalized
{
    ...
    for (NSMutableArray *row in table.cells) {
        for (int i = 0; i < row.count; i++) {
            row[i] = ((MTMathList *)row[i]).finalized;
        }
    }
    return table;
}
...
- (void)setCell:(MTMathList *)list forRow:(NSInteger)row column:(NSInteger)column
{
    ...
    NSMutableArray *rowArray = _cells[row];
    if (rowArray.count <= column) {
        // Add more columns
        for (NSInteger i = rowArray.count; i < column; i++) {
            rowArray[i] = [[[MTMathList alloc] init] autorelease];
        }
    }
...
- (MTColumnAlignment)getAlignmentForColumn:(NSInteger)column
{
    if (self.alignments.count <= column) {
        ...
        return ((NSNumber *)self.alignments[column]).integerValue;
    }
}
...
+ (instancetype)mathListWithAtomsArray:(NSArray *)atoms
{
    MTMathList *list = [[MTMathList alloc] init];
...
- (void)addAtom:(MTMathAtom *)atom
{
    ...
    if (![self isAtomAllowed:atom]) {
        @throw [NSException exceptionWithName:@"Error" reason:[NSString stringWithFormat:@"Cannot add atom of type %@ in a mathlist", typeToText(atom.type)] userInfo:nil];
    }
